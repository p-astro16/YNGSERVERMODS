...
public class RewardSelectionScreen extends ScreenHandler {
    private final Inventory chestInventory;
    private final GameProfile targetProfile;
    private boolean confirmed = false;
    
    public RewardSelectionScreen(int syncId, PlayerInventory playerInventory, GameProfile targetProfile) {
        this(syncId, playerInventory, new SimpleInventory(27), targetProfile);
    }
    
    public RewardSelectionScreen(int syncId, PlayerInventory playerInventory, Inventory chestInventory, GameProfile targetProfile) {
    ...
    private void setupChestButtons() {
        ...
        ItemUtil.setCustomName(info, Text.literal("Plaats items als reward").formatted(Formatting.GOLD, Formatting.BOLD));
    ...
    public void onSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity player) {
        ...
        if (slotIndex == 26 && clicked.getItem() == Items.LIME_WOOL) {
            ...
            // Open time selection screen
            serverPlayer.closeHandledScreen();
            serverPlayer.openHandledScreen(new TimeSelectionScreen.Factory(targetProfile, rewards));
            return;
        }
    ...
    public static class Factory implements NamedScreenHandlerFactory {
        ...
        public Text getDisplayName() {
            return Text.literal("Selecteer Rewards voor " + targetProfile.getName());
        }
        
        @Nullable
        @Override
        public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
            return new RewardSelectionScreen(syncId, playerInventory, targetProfile);
        }
    }
}
